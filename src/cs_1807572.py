...
def dec_decrypt_byte_as_nibbles( nibble_quadruple, key_quadruple ):
    ...
    DC1 = dec_xor(nibble_quadruple, key_quadruple)
    DC2 = dec_swap_last_two(DC1)
    DC3 = dec_s_box(DC2)
    DC4 = dec_xor(DC3, key_quadruple)
    DC5 = dec_subtract_last_two_first_two(DC4)
    DC6 = dec_swap_last_two(DC5)
    DC7 = dec_s_box(DC6)
    DC8 = dec_xor(DC7, key_quadruple)
...
def dec_xor( nibbles, key ):
    ...
    listXor = [0]*4
    for i in range(4):
        listXor[i] = nibbles[i] ^ key[i]# 0,1,2 or 3

    ans = bytearray( listXor )
    return ans
...
def dec_subtract_last_two_first_two( nibbles ):
    ...
    # IF ByteValue is negative as stated with mod 4
    # THEN we catch ValueError and add 4 to rotate values
    try:
        nibbles[0] -= nibbles[2]
    except ValueError:
        nibbles[0] += (4 - nibbles[2])

    try:
        nibbles[1] -= nibbles[3]
    except ValueError:
        nibbles[1] += (4 - nibbles[3])
